	...
	public function index() {
		...
		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/related_options', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$data['action'] = $this->url->link('extension/module/related_options', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		if (isset($this->request->post['module_related_options'])) {
			$data['module_related_options'] = $this->request->post['module_related_options'];
		} elseif ($this->config->get('related_options')) {
			$data['module_related_options'] = $this->config->get('module_related_options');
		} else {
			$data['module_related_options'] = array(
				'price_residue' => 2,
