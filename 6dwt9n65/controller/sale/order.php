...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'invoice'		=> $this->url->link('sale/order/invoice_nutras', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$result['order_id'], true)
			);
		}
	...
	public function invoice_nutras() {
		// Connect Nutras Db
		$remoteDb = new RemoteDB(REMOTE_DB_HOSTNAME, REMOTE_DB_USERNAME, REMOTE_DB_PASSWORD, REMOTE_DB_DATABASE);
		$this->load->language('sale/order');
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
					...
					foreach ($options as $option) {
						...
						$nutras_porduct = $this->model_sale_order->getRemoteProduct($product['product_id'], $option['product_option_value_id']);
						
		 
						$option_data[] = array(
							'name'  => $nutras_porduct['option_name'],
							'value' => $value
						);
					}
					...
					$product_data[] = array(
						'name'     => $nutras_porduct['product_name'],
						'model'    => $product['model'],
						'option'   => $option_data,
						'quantity' => $product['quantity'],
						'price'    => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
				...
				$data['orders'][] = array(
					...
					'invoice_no'       => $invoice_no,
					'date_added'       => date($this->language->get('date_format_short'), strtotime($order_info['date_added'])),
					'date_added_raw'    => date('Y-m-d', strtotime($order_info['date_added'])),
					'store_name'       => $order_info['store_name'],
					'store_url'        => rtrim($order_info['store_url'], '/'),
					'store_address'    => nl2br($store_address),
					'store_email'      => $store_email,
		...
		// $this->response->setOutput($this->load->view('sale/order_invoice_nutras', $data));

		$html = $this->load->view('sale/order_invoice_nutras', $data);
		...
		$dompdf->stream( 'INV-' . $data['orders'][0]['order_id'] . "-".$data['orders'][0]['date_added_raw'].".pdf");
	}
