...
class ModelExtensionDBlogModulePost extends Model {
        ...
        if (isset($data['post_seo_url'])) {
            foreach ($data['post_seo_url'] as $store_id => $language) {
                foreach ($language as $language_id => $keyword) {
                    if (!empty($keyword)) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET route='extension/d_blog_module/post', store_id = '" . (int)$store_id . "', language_id = '" . (int)$language_id . "', query = 'post_id=" . (int)$post_id . "', keyword = '" . $this->db->escape($keyword) . "'");
                    }
                }
            }
        }
    ...
    public function editPost($post_id, $data) {
        ...
        // SEO URL
        $this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE route='extension/d_blog_module/post' AND query = 'post_id=" . (int)$post_id . "'");
