...
class ModelExtensionModuleCron extends Model {
	...
	public function editCron($cron_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "cron` SET `code` = '" . $this->db->escape($data['code']) . "', `cycle` = '" . $this->db->escape($data['cycle']) . "', `action` = '" . $this->db->escape($data['action']) . "',  `date_modified` = NOW() WHERE cron_id = '" . (int)$cron_id . "'");
	}
