...
class ControllerBlogHome extends Controller {
	...
	public function index() { 
		...
		if ($page > 1) {
		  $title_suffix = ' - ' .sprintf($this->language->get('page_number'), $page);
		} else {
		  $title_suffix = '';
		}
		...
		$data['breadcrumbs'] = array();
			...
			'href'      => $this->url->link('common/home')
	      	...
	      	$data['breadcrumbs'][] = array(
			'text'      => $this->language->get('text_blog'). $title_suffix,
    	...
    	foreach ($results as $result) {
			...
			'title' => (mb_strlen($result['title']) >55) ? mb_substr(trim($result['title']), 0,55). '...': $result['title'],
		...
		$this->document->setTitle($blog_page_title[$this->config->get('config_language_id')]. $title_suffix);
		} else {
		$this->document->setTitle($this->language->get('text_blog'). $title_suffix );
		}
		...
		$data['heading_title'] = $blog_title[$this->config->get('config_language_id')]. $title_suffix;
		} else {
		$data['heading_title'] = $this->language->get('text_blog'). $title_suffix;
		}
		...
		if($filter_tag){
		$data['heading_title'] = $this->language->get('text_filter_by') . $filter_tag. $title_suffix;
		$this->document->setTitle($this->language->get('text_filter_by') . $filter_tag. $title_suffix);
		$data['description'] = false;
		}
