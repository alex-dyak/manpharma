<?php
//  
class ControllerExtensionModuleOrderReminder extends Controller {
	

	public function __construct($params) {
		parent::__construct($params);
		$this->max_reminders = 1;
    ...
    public function index() {
        ...
        $this->load->model('checkout/order');
       	...
       	$orders = $this->model_checkout_order->getForgottenOrders(20);
	 
		echo "Orders found: " . count($orders) . PHP_EOL;
        foreach ($orders as $order) {
			$count = $this->model_checkout_order->getOrderReminderCounter($order['order_id'], "1 DAY");
			echo $order['order_id'] . " reminder count: " . $count . PHP_EOL;
			if ($count >= $this->max_reminders) {
				continue;
			}  
			
			$this->model_checkout_order->addOrderReminderCounter($order['order_id']);
            $order_info = $this->model_checkout_order->getOrder($order['order_id']);
            $this->sendNotification($order_info, 20);
			echo "Reminder sent: " . $order['order_id'] . PHP_EOL;
        }
    }

    private function sendNotification($order_info, $order_status_id ) {
		...
		$language->load('mail/order_reminder');
		...
		$data['text_footer'] = sprintf($language->get('text_footer'), $this->config->get('config_email'));
		...
		$data['comment'] = sprintf($language->get('bank_transfer_details'), 'INV' . str_pad($order_info['order_id'],  6, "0")); 
		...
		$mail->setSubject(html_entity_decode($language->get('text_subject'), ENT_QUOTES, 'UTF-8'));
		$mail->setHtml($this->load->view('mail/order_reminder', $data));
		$mail->send();
	}
