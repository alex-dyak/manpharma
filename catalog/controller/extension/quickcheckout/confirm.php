...
class ControllerExtensionQuickCheckoutConfirm extends Controller { 
	public function index($save_order = false) {
		$redirect = '';
		...
		if (!$redirect || $save_order) {
			...
			if ($this->customer->isLogged()) {
				...
				$order_data['customer_group_id'] = isset($this->session->data['guest']['customer_group_id']) ? $this->session->data['guest']['customer_group_id'] : 0;
