...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		if ($order_status_id == 20 && $order_info['payment_code'] == 'bank_transfer') {
	...
	public function edit($order_info, $order_status_id, $comment) {
		...
		$data['order_id'] = 'INV' . str_pad($order_info['order_id'],  6, "0");
		$data['date_added'] = date($language->get('date_format_short'), strtotime($order_info['date_added']));
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			$data['email_customer'] = $order_info['email'];
			$data['order_id'] = $order_info['order_id'];
			$data['firstname'] = !$order_info['shipping_firstname'] ? $order_info['payment_firstname'] : $order_info['shipping_firstname'];
			$data['lastname']  = !$order_info['shipping_lastname'] ? $order_info['payment_lastname'] : $order_info['shipping_lastname'];
		...
		$data['shipping_address_aler'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
