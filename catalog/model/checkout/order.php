...
class ModelCheckoutOrder extends Model
{
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false)
	{
		...
		if ($order_info) {
			...
			// Update user group
			// if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_complete_status')))) {

			//     $config_complete_status = implode(',', $this->config->get('config_complete_status'));
			//     $query = $this->db->query("SELECT COUNT(*) AS cnt FROM " . DB_PREFIX . "order WHERE order_status_id IN (" . $config_complete_status . ") AND customer_id = '". $order_info['customer_id'] ."'");
			//     $cnt = $query->row['cnt'];
			//     #die("SELECT COUNT(*) AS cnt FROM " . DB_PREFIX . "order WHERE order_status_id IN (" . $config_complete_status . ") AND customer_id = '". $order_info['customer_id'] ."'");
			//     if ($cnt == 1) {
			//         $customer_group_id = 3;
			//     } elseif ($cnt == 2) {
			//         $customer_group_id = 4;
			...
			//     $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '". (int)$customer_group_id ."' WHERE customer_id = '". $order_info['customer_id'] ."'");
			// }
		}
	}

	public function getForgottenOrders($order_status_id, $period = "3 DAY")
	{
		$query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE `order_status_id` = '" . (int)$order_status_id . "' AND `date_added` < DATE_SUB(NOW(), INTERVAL " . $period . ")");

		return $query->rows;
	}

	public function getOrderReminderCounter($order_id)
	{
		$query = $this->db->query("SELECT COUNT(*) AS cnt FROM `" . DB_PREFIX . "order_reminder` WHERE `order_id` = '" . (int)$order_id . "'");
		return $query->row['cnt'];
	}

	public function addOrderReminderCounter($order_id)
	{
		return $this->db->query("INSERT INTO `" . DB_PREFIX . "order_reminder` SET `order_id` = '" . (int)$order_id . "'");
	}

	public function getCompletedOrders($order_status_id)
	{
		$query = $this->db->query("SELECT o.`order_id` FROM `" . DB_PREFIX . "order` AS o WHERE o.`order_status_id` = '" . (int)$order_status_id . "' AND o.`order_id` NOT IN (SELECT oc.`order_id` FROM `" . DB_PREFIX . "order_connected` AS oc) ORDER BY o.`order_id` DESC");

		return $query->rows;
	}


	public function getConnectedProduct($option_id, $product_option_value_id)
	{
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_option_value` WHERE `product_option_value_id` = '" . $product_option_value_id . "' AND `option_id` = '" . $option_id . "' LIMIT 1");
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_connected` WHERE `product_id` = '" . $product_id . "' AND `option_id` = '" . $option_id . "' AND `option_value_id` = '" . $option_value_id . "' LIMIT 1");
