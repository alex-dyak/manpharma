	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['flat'] = array(
				...
				'cost'         => intval($cart_total) > 79 ? 0 : $this->config->get('shipping_flat_cost'),
				'tax_class_id' => $this->config->get('shipping_flat_tax_class_id'),
				'text'         => intval($cart_total) > 79 ? $this->language->get('text_free') : $this->currency->format($this->tax->calculate($this->config->get('shipping_flat_cost'), $this->config->get('shipping_flat_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
