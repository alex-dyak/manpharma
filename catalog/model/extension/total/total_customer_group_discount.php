	...
	public function getTotal($total) {
		if($user_id = $this->customer->isLogged()){
			...
			if($customerDiscount != 0){
				...
				if($this->config->get('total_total_customer_group_discount_tax')){
					if($this->config->get('total_total_customer_group_discount_special')){
			...
			if($this->config->get('total_total_customer_group_discount_show') == 1 || ($customerDiscount != 0 && $this->config->get('total_total_customer_group_discount_show') == 2)){
				...
				$total['totals'][] = array(
					...
					'value'      => -$subtraction,
					'sort_order' => (int)$this->config->get('total_total_customer_group_discount_sort_order')
				);
